...
const PAYMENT_OPTIONS: PaymentOption[] = [
  // ── Ví điện tử ──
  {
    ...
    group: "wallet", color: "#fff", bg: "#ae2070",
    icon: <span style={{ fontSize: 22, fontWeight: 900, letterSpacing: -1 }}>M</span>,
  },
  {
    id: "zalopay", label: "ZaloPay", sub: "Ví ZaloPay",
    group: "wallet", color: "#fff", bg: "#0068ff",
    icon: <span style={{ fontSize: 20, fontWeight: 900 }}>Z</span>,
  },
  {
    id: "vnpay", label: "VNPay", sub: "Ví VNPay QR",
    group: "wallet", color: "#fff", bg: "#e31837",
    icon: <span style={{ fontSize: 18, fontWeight: 900 }}>VN</span>,
  },
  // ── Ngân hàng ──
  {
    id: "bank_vcb", label: "Vietcombank", sub: "ATM / Internet Banking",
    group: "bank", color: "#fff", bg: "#006633",
    icon: <span style={{ fontSize: 14, fontWeight: 700 }}>VCB</span>,
  },
  {
    id: "bank_tcb", label: "Techcombank", sub: "ATM / Internet Banking",
    group: "bank", color: "#fff", bg: "#d0021b",
    icon: <span style={{ fontSize: 14, fontWeight: 700 }}>TCB</span>,
  },
  {
    id: "bank_acb", label: "ACB", sub: "ATM / Internet Banking",
    group: "bank", color: "#fff", bg: "#005c9e",
    icon: <span style={{ fontSize: 14, fontWeight: 700 }}>ACB</span>,
  },
  {
    id: "bank_mb", label: "MB Bank", sub: "ATM / Internet Banking",
    group: "bank", color: "#fff", bg: "#003087",
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#64748b" }}>
              Tổng thanh toán:{" "}
              <strong style={{ color: "#f97316" }}>{formatVnd(finalTotal)} ₫</strong>
            </p>
          </div>
        ...
        <div style={{ padding: "20px 24px", maxHeight: "60vh", overflowY: "auto" }}>
          ...
          {/* Ví điện tử */}
          <p style={sectionLabel}>🪙 Ví điện tử</p>
          ...
          {/* Ngân hàng */}
          <p style={{ ...sectionLabel, marginTop: 20 }}>🏦 Ngân hàng</p>
          <div style={optionGrid}>
            {banks.map((opt) => (
              <PaymentOptionCard
                key={opt.id}
        ...
        <div style={{
          ...
          <p style={{ margin: 0, textAlign: "center", fontSize: 12, color: "#475569" }}>
            🔒 Giao dịch được mã hóa SSL — An toàn 100%
          </p>
        </div>
      </div>
    </>,
...
}: { opt: PaymentOption; selected: boolean; onSelect: () => void }) {
  return (
    <button
      type="button"
      onClick={onSelect}
...
function CartPage() {
  ...
  const totalAfterCourseDiscount = cartItems.reduce((sum, c) => {
    ...
    const current  = c.currentPrice != null
      ? Number(String(c.currentPrice).replace(/[^0-9.]/g, ""))
      : null;
    const eff = current != null && current > 0 && current < original ? current : original;
    return sum + eff;
  }, 0);
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      const res = await axiosInstance.post("/courseCreation/apply-coupon", {
        user_id: userId, code, order_total: totalAfterCourseDiscount,
      });
      const data = res.data;
      setCouponResult({
        couponId: data.couponId, code: data.coupon.code,
        type: data.coupon.type,  value: data.coupon.value,
        discount: data.discount, finalTotal: data.finalTotal,
  ...
  const handleRemoveCoupon = () => {
    setCouponResult(null); setCouponCode(""); setCouponStatus("idle"); setCouponError("");
  ...
  const handleConfirmPayment = async (method: PaymentMethod) => {
    ...
    try {
      ...
      const methodLabel = PAYMENT_OPTIONS.find((p) => p.id === method)?.label ?? method;
      toast.success(
        "Thanh toán thành công! 🎉",
        `${methodLabel} · ${cartItems.length} khóa học · ${formatVnd(finalTotal)} ₫`,
      );
    ...
    return (
      <div className="cart-page">
        <div className="cart-empty">
          ...
          <p className="cart-empty__sub">Hãy thêm một khóa học để bắt đầu hành trình học tập!</p>
          <Link to="/" className="cart-empty__btn">Khám phá khóa học</Link>
        </div>
      </div>
    );
  }
      ...
      <div className="cart-page__inner">
        <div className="cart-page__header">
          <div>
            ...
            <p className="cart-page__count">{cartItems.length} khóa học trong giỏ hàng</p>
          </div>
          {cartItems.length > 1 && (
        ...
        <div className="cart-page__layout">
          ...
          <div className="cart-list">
            {cartItems.map((course) => {
              ...
              const current  = course.currentPrice != null
              ...
              const discountPct    = hasDiscount ? Math.round(((price - current!) / price) * 100) : 0;
                ...
                <div key={courseId} className="cart-item">
                  <Link to={`/course-detail/${courseId}`} className="cart-item__img-wrap">
                    <img className="cart-item__img"
                      src={getCourseImageUrl(course.path)} alt={course.title}
                      onError={(e) => {
                        (e.target as HTMLImageElement).src = "https://s.udemycdn.com/course/750x422/placeholder.jpg";
                      }} />
                  </Link>
                  <div className="cart-item__info">
                    ...
                    <p className="cart-item__author">Bởi <span>{course.author}</span></p>
          ...
          <div className="cart-summary">
            ...
            <button type="button" className="cart-summary__checkout-btn"
              ...
              <span className="cart-summary__checkout-price">{formatVnd(finalTotal)} ₫</span>
            </button>

            {/* ── Payment methods bar ── */}
            <div className="cart-summary__methods">
              <p className="cart-summary__methods-label">Chấp nhận thanh toán</p>
              <div className="cart-summary__methods-icons">
                {PAYMENT_OPTIONS.map((p) => (
                  <div key={p.id} className="cart-summary__method-chip" title={p.label}
            ...
            <p className="cart-summary__guarantee">🔒 Thanh toán bảo mật SSL · Hoàn tiền 30 ngày</p>
            ...
            <div className="cart-summary__coupon">
              ...
              {couponResult ? (
                <div className="coupon-applied">
                  ...
                  <button type="button" className="coupon-applied__remove"
                    onClick={handleRemoveCoupon} aria-label="Xóa mã">✕</button>
                </div>
              ) : (
                <div className="cart-summary__coupon-row">
                  <input ref={inputRef} type="text"
                    ...
                    onChange={(e) => {
                      ...
                      if (couponStatus === "error") { setCouponStatus("idle"); setCouponError(""); }
                    }}
              ...
              {couponStatus === "idle" && !couponCode && (
                <p className="coupon-hint">
                  ...
                  {["WELCOME20", "SAVE50K", "CTUET2025"].map((c) => (
                    <button key={c} type="button" onClick={() => setCouponCode(c)}>{c}</button>
                  ))}
                </p>
              )}
            </div>
