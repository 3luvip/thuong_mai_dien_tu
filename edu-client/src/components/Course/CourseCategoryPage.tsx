...
// Pagination server-side — không load toàn bộ, chỉ fetch trang hiện tại
...
export default function CourseCategoryPage() {
  ...
  const keywordParam  = urlParams.get("keyword")  ?? "";   // từ TopicSlider
  ...
  // Khởi tạo keyword từ URL (khi navigate từ TopicSlider)
  ...
  // Reset khi URL thay đổi (category hoặc keyword từ TopicSlider)
  useEffect(() => {
    setPage(1);
    setFilterLevels([]);
  ...
  const fetchCourses = useCallback(async () => {
    ...
    try {
      ...
      // Client-side: sort (server chỉ sort created_at, ta sort thêm)
      if (sort === "price_asc") data = [...data].sort((a, b) => (a.currentPrice || a.price) - (b.currentPrice || b.price));
      else if (sort === "price_desc") data = [...data].sort((a, b) => (b.currentPrice || b.price) - (a.currentPrice || a.price));
      else if (sort === "rating") data = [...data].sort((a, b) => getMockRating(b.id).value - getMockRating(a.id).value);

      // Client-side: filter rating (không có trường thật nên filter mock)
      if (filterRating !== null) {
        data = data.filter((c) => getMockRating(c.id).value >= filterRating!);
      }

      // Client-side: multi-level filter nếu chọn nhiều hơn 1
      if (filterLevels.length > 1) {
        data = data.filter((c) => filterLevels.includes(c.level));
      }
  ...
  const toggleLevel = (lv: string) => {
    setPage(1);
    setFilterLevels((prev) => prev.includes(lv) ? prev.filter((l) => l !== lv) : [...prev, lv]);
  };
        ...
        <div style={pg.inner}>
          <nav style={pg.breadcrumb}>
            ...
            {categoryParam && (
              <>
                <span style={pg.bcSep}>›</span>
                <span style={pg.bcCur}>{categoryParam}</span>
              </>
            )}
            {!categoryParam && keywordParam && (
              <>
                <span style={pg.bcSep}>›</span>
                <span style={pg.bcCur}>"{keywordParam}"</span>
              </>
            )}
          </nav>
          <h1 style={pg.heroTitle}>
            {categoryParam
              ...
              : keywordParam
              ? `Kết quả cho "${keywordParam}"`
              : "Tất cả khóa học"}
          </h1>
          <p style={pg.heroSub}>
            <strong style={{ color: "#e5e7eb" }}>{pagination.total.toLocaleString("vi-VN")}</strong>
            {" "}khóa học
          ...
          <div style={pg.layout}>
            ...
            <div style={pg.courseList}>
              <div style={pg.listHead}>
                <h2 style={pg.listTitle}>
                  {categoryParam || "Tất cả khóa học"}
                </h2>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span style={{ color: "#9ca3af", fontSize: 13 }}>Sắp xếp theo</span>
                  <select style={pg.sortSel} value={sort} onChange={(e) => { setSort(e.target.value as SortKey); setPage(1); }}>
              ...
              <p style={{ fontSize: 13, color: "#9ca3af", margin: "0 0 12px" }}>
                Hiển thị <strong style={{ color: "#e5e7eb" }}>{courses.length}</strong> / <strong style={{ color: "#e5e7eb" }}>{pagination.total.toLocaleString("vi-VN")}</strong> khóa học
                ...
                : courses.length === 0
                  ? (
                    <div style={pg.empty}>
                      ...
                      <button type="button" style={pg.clearBtn2} onClick={() => { clearFilters(); setKeyword(""); }}>Xóa bộ lọc</button>
                    </div>
                  )
                  : courses.map((c) => <ListItem key={c.id} course={c} />)
              }
    ...
    <div style={pag.wrap}>
      ...
      {pages.map((p, i) =>
        p === "..." ? (
          ...
          <button
            key={p}
            style={{ ...pag.btn, ...(p === current ? pag.btnActive : {}) }}
  ...
  return (
    <Link
      ...
      style={{
        ...
        color: "inherit", padding: "16px 0", borderBottom: "1px solid #1e293b",
        background: hov ? "rgba(255,255,255,0.02)" : "transparent", transition: "background .15s",
      }}
      onMouseEnter={() => setHov(true)}
      onMouseLeave={() => setHov(false)}
    >
      <div style={{ flexShrink: 0, width: 200, aspectRatio: "16/9", borderRadius: 8, overflow: "hidden" }}>
        <img
          src={getCourseImageUrl(course.path)}
          alt={course.title}
          style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", transform: hov ? "scale(1.04)" : "scale(1)", transition: "transform .3s" }}
          onError={(e) => { (e.target as HTMLImageElement).src = "https://s.udemycdn.com/course/750x422/placeholder.jpg"; }}
        />
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <h3 style={{ fontSize: 15, fontWeight: 600, color: "#f1f5f9", margin: "0 0 4px", lineHeight: 1.4 }}>
          {course.title}
        </h3>
        <p style={{ fontSize: 12, color: "#9ca3af", margin: "0 0 4px", lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
          {course.courseSub}
        </p>
...
const pg: Record<string, React.CSSProperties> = {
  ...
  searchInput: {
    ...
    background: "#0f172a", border: "1px solid #374151", borderRadius: 10,
...
// React.CSSProperties yêu cầu import React ở đầu file nếu dùng JSX — đã có vì dùng useState