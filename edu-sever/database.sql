...
-- ─── COURSE_INSTRUCTIONS (phải tạo trước courses vì courses FK đến đây) ───
...
CREATE TABLE IF NOT EXISTS notifications (
    ...
    user_id    VARCHAR(36)  NOT NULL,
    type       ENUM(
                 'course_added',      -- instructor thêm khóa học mới
                 'discount',          -- khóa học giảm giá
                 'coupon',            -- mã giảm giá mới
                 'system',            -- thông báo hệ thống
                 'reminder'           -- nhắc nhở học tập
               ) NOT NULL DEFAULT 'system',
    title      VARCHAR(255) NOT NULL,
    body       TEXT         NOT NULL,
    -- deep-link: click thông báo → navigate đến đâu
...
CREATE TABLE IF NOT EXISTS coupons (
    ...
    -- Giới hạn toàn hệ thống: tổng số lần mã được dùng trên TẤT CẢ user
    total_limit     INT                     NOT NULL DEFAULT 100,
    -- Giới hạn mỗi user: mỗi user chỉ được dùng mã này bao nhiêu lần
    per_user_limit  INT                     NOT NULL DEFAULT 1,
    -- Đơn tối thiểu để dùng mã
    min_order       DECIMAL(10,2)           NOT NULL DEFAULT 0,
    -- Giảm tối đa (chỉ áp dụng type=percent)
...
CREATE TABLE IF NOT EXISTS coupon_usages (
    ...
    -- discount_amount: số tiền đã giảm trong lần dùng này (để audit)
...
CREATE TABLE IF NOT EXISTS lectures (
    ...
    duration_sec INT           NOT NULL DEFAULT 0,   -- thời lượng (giây)
    is_preview   TINYINT(1)    NOT NULL DEFAULT 0,   -- cho phép xem trước miễn phí
